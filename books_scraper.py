...
def scrape_books():
    ...
    for books_div in soup.find_all("div", class_="elementList"):
        ...
        author_tag = books_div.find("a", class_="bookAuthor")
        info_tag = books_div.find("a", class_="bookInfo")

    if title_tag and author_tag:
        title = title_tag.text.strip()
        author = author_tag.text.strip()

        full_link = f"{title_tag["href"]}"
        ...
        genre_soup = BeautifulSoup(genre_response, "html.parser")
        genres = [
            genre.get_text(strip=True)
            for genre in genre_soup.find_all("span", class_="BookPageMetadataSection__genrePlainText")
        ]
